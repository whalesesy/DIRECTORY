...
<div class="space-y-6 max-w-2xl">
    <div>
        <h2 class="text-3xl font-black text-kisii-blue tracking-tight">Edit Staff Member</h2>
        <p class="text-sm text-kisii-text-muted mt-1">Modify contact details for this staff member.</p>
    </div>

    <form action="{{ route('admin.staff.update', $staff->id) }}" method="POST" class="bg-white rounded-3xl border border-kisii-border p-6 shadow-sm space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" id="name" required value="{{ old('name', $staff->name) }}" placeholder="e.g. Susan Nyamweya" class="w-full px-4 py-3 rounded-xl border border-kisii-border focus:outline-none focus:ring-2 focus:ring-kisii-blue/25 focus:border-kisii-blue text-sm shadow-sm transition-all">
        ...
        <div>
            ...
            <input type="text" name="role" id="role" required value="{{ old('role', $staff->role) }}" placeholder="e.g. Chief of Staff" class="w-full px-4 py-3 rounded-xl border border-kisii-border focus:outline-none focus:ring-2 focus:ring-kisii-blue/25 focus:border-kisii-blue text-sm shadow-sm transition-all">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="ext" id="ext" value="{{ old('ext', $staff->ext) }}" placeholder="e.g. 101" class="w-full px-4 py-3 rounded-xl border border-kisii-border focus:outline-none focus:ring-2 focus:ring-kisii-blue/25 focus:border-kisii-blue text-sm shadow-sm transition-all">
            ...
            <div>
                ...
                <input type="number" name="sort_order" id="sort_order" required value="{{ old('sort_order', $staff->sort_order) }}" class="w-full px-4 py-3 rounded-xl border border-kisii-border focus:outline-none focus:ring-2 focus:ring-kisii-blue/25 focus:border-kisii-blue text-sm shadow-sm transition-all">
        ...
        <div class="border-t border-kisii-border pt-4 space-y-4">
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $staff->email) }}" placeholder="e.g. officer@kisii.go.ke" class="w-full px-4 py-3 rounded-xl border border-kisii-border focus:outline-none focus:ring-2 focus:ring-kisii-blue/25 focus:border-kisii-blue text-sm shadow-sm transition-all">
        ...
        <div class="flex flex-col sm:flex-row gap-4 pt-2 border-t border-kisii-border">
            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_senior" id="is_senior" value="1" {{ old('is_senior', $staff->is_senior) ? 'checked' : '' }} class="w-4 h-4 rounded text-kisii-blue border-kisii-border focus:ring-kisii-blue/25">
                <label for="is_senior" class="text-sm font-semibold text-kisii-text">⭐ Mark as Senior Leader</label>
            </div>
            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $staff->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded text-kisii-blue border-kisii-border focus:ring-kisii-blue/25">
