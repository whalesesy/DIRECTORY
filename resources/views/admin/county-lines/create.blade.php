...
<div class="space-y-6 max-w-2xl">
    <div>
        <h2 class="text-3xl font-black text-kisii-blue tracking-tight">Add Hotline</h2>
        <p class="text-sm text-kisii-text-muted mt-1">Create a new county hotline support line.</p>
    </div>

    <form action="{{ route('admin.county-lines.store') }}" method="POST" class="bg-white rounded-3xl border border-kisii-border p-6 shadow-sm space-y-5">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="label" id="label" required value="{{ old('label') }}" placeholder="e.g. Safaricom" class="w-full px-4 py-3 rounded-xl border border-kisii-border focus:outline-none focus:ring-2 focus:ring-kisii-blue/25 focus:border-kisii-blue text-sm shadow-sm transition-all">
        ...
        <div>
            ...
            <input type="text" name="number" id="number" required value="{{ old('number') }}" placeholder="e.g. 0709727000" class="w-full px-4 py-3 rounded-xl border border-kisii-border focus:outline-none focus:ring-2 focus:ring-kisii-blue/25 focus:border-kisii-blue text-sm shadow-sm transition-all">
